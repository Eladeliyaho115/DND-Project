...
import { api } from './../api/axiosClient'; // ה-Axios instance של ה-Backend שלך
import type { Character } from '../types/character';

// קריאת בלייב - מיועד ל-Hook של ה-5 שניות
...
// שמירה / הוספה ל-DB - מיועד לכפתור הוספת דמות ושמירה בסוף סשן
export const saveOrSyncDndBeyondCharacter = async (beyondId: string): Promise<Character> => {
  const response = await api.post(`/characters/beyond/${beyondId}/save`);
  // השרת מחזיר { message: "...", character: {...} }
