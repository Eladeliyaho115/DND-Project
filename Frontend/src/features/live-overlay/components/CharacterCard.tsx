...
// export const CharacterCard: React.FC<CharacterCardProps> = ({ character: initialCharacter, onOpenDetails }) => {
//   // 1. נחלץ את ה-ID (נעזר ב-beyondId מ-Prisma / Backend)
//   const characterId = initialCharacter.dndCharacterId || initialCharacter.dndCharacterId || '';

//   // 2. נעביר ל-Hook את ה-ID התקין
//   const { character: liveCharacter, loading, error } = useDndCharacter(characterId, 5000);

//   // 3. נשתמש במידע מה-LIVE במידה וקיים
//   const displayChar = characterId && liveCharacter 
//     ? liveCharacter 
//     : initialCharacter;
//   const hpPercentage = Math.max(
//     0, 
//     Math.min(100, (displayChar.hp.current / displayChar.hp.max) * 100)
//   );
...
//   // חישוב Fallback עבור Proficiency ו-Passive Skills במידה והנתונים אינם קיימים בבלוק הראשי
//   const proficiencyBonus = displayChar.proficiencyBonus ?? Math.ceil(1 + (displayChar.level || 1) / 4);
//   const wisMod = Math.floor(((displayChar.stats?.wis ?? 10) - 10) / 2);
//   const intMod = Math.floor(((displayChar.stats?.int ?? 10) - 10) / 2);

//   const passivePerception = displayChar.passiveSkills?.perception ?? (10 + wisMod);
//   const passiveInvestigation = displayChar.passiveSkills?.investigation ?? (10 + intMod);
...
//             Lvl {displayChar.level}
//           </span>

//           {/* Proficiency Bonus Badge - חדש! */}
//           <span 
...
//             <h3 className="text-lg font-bold text-white truncate">{displayChar.name}</h3>
...
//           <p className="text-sm text-indigo-400 font-medium truncate">{displayChar.class}</p>
//           <p className="text-xs text-slate-400 truncate">שחקן: {displayChar.player}</p>
//         </div>
//       </div>
...
//           </span>
//           <span>
//             <strong className="text-white">{displayChar.hp.current}</strong>
//             <span className="text-slate-500"> / {displayChar.hp.max}</span>
//             {displayChar.hp.temp && displayChar.hp.temp > 0 ? (
//               <span className="text-sky-400 text-[10px] ml-1">({displayChar.hp.temp}+)</span>
//             ) : null}
//           </span>
//         </div>
...
//             AC
//           </div>
//           <div className="text-lg font-bold text-white mt-0.5">{displayChar.ac}</div>
//         </div>
...
//             מהירות
//           </div>
//           <div className="text-lg font-bold text-white mt-0.5">{displayChar.speed || 30}ft</div>
//         </div>
//       </div>
...
//         {[
//           { label: 'STR', val: displayChar.stats?.str },
//           { label: 'DEX', val: displayChar.stats?.dex },
//           { label: 'CON', val: displayChar.stats?.con },
//           { label: 'INT', val: displayChar.stats?.int },
//           { label: 'WIS', val: displayChar.stats?.wis },
//           { label: 'CHA', val: displayChar.stats?.cha },
//         ].map((stat) => (
//           <div key={stat.label} className="bg-slate-950/60 rounded py-1 border border-slate-800/40">
//             {React.createElement('div', { className: 'text-[9px] text-slate-400 font-bold' }, stat.label)}
//             {React.createElement('div', { className: 'text-xs font-bold text-slate-200' }, getMod(stat.val))}
//           </div>
//         ))}
//       </div>

//       {/* Passive Skills Bar - חדש! */}
...
//       {/* כפתור לפתיחת חלון הפרטים המורחב */}
//       <button
...
export const CharacterCard: React.FC<CharacterCardProps> = ({ character: initialCharacter, onOpenDetails }) => {
  // 1. חילוץ בטוח של ה-ID עבור ה-Hook (תומך ב-beyondId וב-dndCharacterId)
  const characterId = initialCharacter?.dndCharacterId || initialCharacter?.dndCharacterId || '';

  // 2. קריאה ל-Hook (תומך ב-Hook מקבל ID וב-Hook ללא פרמטרים)
  const hookResult = useDndCharacter(characterId, 5000) as any;
  const liveCharacter = hookResult?.character ?? null;
  const loading = hookResult?.loading ?? false;
  const error = hookResult?.error ?? null;

  // 3. בחירת האובייקט להצגה (Live קודם, אחרת Initial)
  const displayChar = (characterId && liveCharacter) ? liveCharacter : initialCharacter;

  // 4. חילוץ בטוח של HP למניעת שגיאות Undefined
  const currentHp = displayChar?.hp?.current ?? (displayChar as any)?.currentHp ?? 0;
  const maxHp = displayChar?.hp?.max ?? (displayChar as any)?.maxHp ?? 1;
  const tempHp = displayChar?.hp?.temp ?? (displayChar as any)?.tempHp ?? 0;
  ...
  // 5. חילוץ בטוח של Stats ו-Passive Skills
  const stats = displayChar?.stats || {};
  const level = displayChar?.level || 1;

  const proficiencyBonus = displayChar?.proficiencyBonus ?? Math.ceil(1 + level / 4);
  const wisMod = Math.floor(((stats.wis ?? 10) - 10) / 2);
  const intMod = Math.floor(((stats.int ?? 10) - 10) / 2);
    ...
    <div className="w-80 rounded-2xl bg-slate-900 border border-slate-800 text-slate-100 shadow-xl overflow-hidden hover:border-indigo-500/50 transition-all duration-300 flex flex-col">
      ...
      <div className="relative h-28 bg-gradient-to-r from-slate-950 via-slate-900 to-indigo-950 p-4 flex items-center gap-4">
        <div className="relative">
          <div className="w-20 h-20 rounded-xl overflow-hidden border-2 border-indigo-500/30 bg-slate-800 flex items-center justify-center shrink-0">
            {displayChar?.avatarUrl ? (
              <img src={displayChar.avatarUrl} alt={displayChar.name || 'Character'} className="w-full h-full object-cover" />
            ) : (
              <svg className="w-10 h-10 text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
              </svg>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-1">
            <h3 className="text-lg font-bold text-white truncate">{displayChar?.name || 'דמות ללא שם'}</h3>
          ...
          <p className="text-sm text-indigo-400 font-medium truncate">{displayChar?.class || 'לא הוגדר מקצוע'}</p>
          <p className="text-xs text-slate-400 truncate">שחקן: {displayChar?.player || 'לא הוגדר'}</p>
        </div>
      </div>

      {/* מד חיים (HP Bar) - תצוגה מוגנת */}
      <div className="px-4 pt-3 pb-2 bg-slate-900/80 border-t border-slate-800">
        <div className="flex justify-between items-center text-xs font-semibold mb-1">
          <span className="flex items-center gap-1 text-rose-400">
            <svg className="w-3.5 h-3.5 fill-current" viewBox="0 0 24 24">
      ...
      <div className="grid grid-cols-3 gap-2 px-4 py-3 bg-slate-950/40 border-y border-slate-800/60 text-center">
        <div className="p-1.5 bg-slate-900/60 rounded-lg border border-slate-800">
          ...
          <div className="text-lg font-bold text-white mt-0.5">{displayChar?.ac ?? 10}</div>
        </div>
      ...
      <div className="grid grid-cols-6 gap-1 px-4 py-2 text-center bg-slate-900">
        {[
          ...
          <div key={stat.label} className="bg-slate-950/60 rounded py-1 border border-slate-800/40">
            ...
            <div className="text-xs font-bold text-slate-200">{getMod(stat.val)}</div>
          </div>
        ))}
      </div>
