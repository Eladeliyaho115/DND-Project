// backend/src/services/character/characterParser.ts

export const parseBeyondToFrontend = (data: any) => {
  // 1. Stats & Modifiers
  ...
  const getStatValue = (statId: number): number => {
    ...
    const bonus =
      data.bonusStats?.find((s: any) => s.id === statId)?.value || 0;

    const override = data.overrideStats?.find(
      (s: any) => s.id === statId,
    )?.value;

    // ?? במקום || כדי שגם override של 0 יעבוד
  ...
  const totalLevel =
    data.classes?.reduce((acc: number, c: any) => acc + (c.level || 0), 0) || 1;

  const proficiencyBonus =
    data.proficiencyBonus || Math.ceil(1 + totalLevel / 4);
  ...
  if (equippedArmor) {
    ...
    if (armorType === 1) {
      calculatedAc = armorClass + dexModifier;
    } else if (armorType === 2) {
      calculatedAc = armorClass + Math.min(2, dexModifier);
    } else if (armorType === 3) {
      calculatedAc = armorClass;
    }
  }

  const finalAc = data.overrideArmorClass ?? calculatedAc;

  // 3. Passive Skills & Senses
  ...
  const getPassiveSkill = (skillId: number, defaultStatMod: number): number => {
    ...
    if (customOverride?.value !== undefined) {
      return customOverride.value;
    }
    ...
    const hasExpertise = allModifiers.some(
      (m: any) =>
        m.type === "expertise" && m.subType?.toLowerCase() === skillName,
    );

    const hasProficiency = allModifiers.some(
      (m: any) =>
        (m.type === "proficiency" || m.type === "half-proficiency") &&
        m.subType?.toLowerCase() === skillName,
    );

    const passiveBonus = allModifiers
      .filter(
        (m: any) =>
          m.type === "passive-bonus" && m.subType?.toLowerCase() === skillName,
      )
  ...
  const classSpells =
    data.classSpells?.flatMap((cs: any) => cs.spells || []) || [];

  const raceSpells = data.spells?.race || [];

  const featSpells = data.spells?.feat || [];

  const spells = [...classSpells, ...raceSpells, ...featSpells].map(
    (s: any) => ({
  ...
  const classFeatures =
    data.classes
      ?.flatMap((c: any) => c.classFeatures || [])
      .map((cf: any) => cf.definition) || [];

  const feats = data.feats?.map((f: any) => f.definition) || [];

  const raceTraits =
    data.race?.racialTraits?.map((t: any) => t.definition) || [];
  ...
  return {
    ...
    player: data.username || "לא הוגדר",

    class: primaryClass,

    race: data.race?.fullName || data.race?.baseName || "Unknown Race",
